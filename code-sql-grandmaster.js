(function () {
  ...
  const rows = [
    ...
    ["gm-java-016", "Java", "code", "mid", "Java에서 아직 완성되지 않은 메서드를 포함할 수 있고 직접 객체 생성이 불가능한 클래스 키워드는?", ["abstract"], "abstract", "abstract class는 미완성 메서드를 포함할 수 있고 new로 직접 만들 수 없다.", ["Java", "abstract"], { answerMode: "literal" }],
    ["gm-java-017", "Java", "code", "mid", "Java에서 부모 클래스를 상속받을 때 사용하는 키워드는?", ["extends"], "extends", "class Child extends Parent 형태로 쓴다.", ["Java", "상속"], { answerMode: "literal" }],
    ...
    ["gm-java-020", "Java", "code", "mid", "Java에서 더 이상 값을 바꾸지 못하게 변수나 메서드, 클래스에 붙이는 키워드는?", ["final"], "final", "final 변수는 재할당할 수 없다.", ["Java", "final"], { answerMode: "literal" }],
    ...
    ["gm-py-006", "Python", "code", "mid", "Python 리스트에서 다른 리스트의 원소들을 하나씩 풀어 뒤에 붙이는 메서드는?", ["extend", "extend()"], "extend", "append는 통째로 한 칸, extend는 원소를 풀어서 추가한다.", ["Python", "리스트"], { answerMode: "literal" }],
    ...
    ["gm-py-009", "Python", "code", "mid", "Python 딕셔너리에서 key와 value 쌍을 순회할 때 자주 쓰는 메서드는?", ["items()", "d.items()"], "items()", "for k, v in d.items(): 형태로 쓴다.", ["Python", "dict"], { answerMode: "literal" }],
    ...
    ["gm-sql-003", "SQL", "sql", "must", "NULL까지 포함해 모든 행 수를 세는 집계 함수 표현은?", ["COUNT(*)"], "COUNT(*)", "COUNT(컬럼)은 NULL을 제외할 수 있지만 COUNT(*)는 행 자체를 센다.", ["SQL", "COUNT"], { answerMode: "sql-literal" }],
